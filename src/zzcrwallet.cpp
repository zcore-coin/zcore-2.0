...
CzZCRWallet::CzZCRWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZZCRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZZCRSeed_deprecated()) {
                    LogPrintf("%s: Updated zZCRseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzZCRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZZCRCount(nCountLastUsed);
    else if (!walletdb.ReadZZCRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzZCRWallet::Lock()
{
    seedMaster = 0;
}

void CzZCRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzZCRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZZCR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzZCRWallet::LoadMintPoolFromDB()
{
...
void CzZCRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzZCRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzZCRWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzZCRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZZCR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzZCRWallet::SeedToZZCR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzZCRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzZCRWallet::GenerateDeterministicZZCR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzZCRWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzZCRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
