...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ZCR.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzZCRReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzZCRReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzZCRReset.connect(boost::bind(NotifyzZCRReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzZCRReset.disconnect(boost::bind(NotifyzZCRReset, this));
