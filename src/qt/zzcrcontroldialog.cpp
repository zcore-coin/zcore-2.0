...
#include "ui_zzcrcontroldialog.h"
...
std::set<std::string> ZZcrControlDialog::setSelectedMints;
std::set<CMintMeta> ZZcrControlDialog::setMints;

ZZcrControlDialog::ZZcrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZZcrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZZcrControlDialog::~ZZcrControlDialog()
{
    delete ui;
}

void ZZcrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZZcrControlDialog::updateList()
{
...
void ZZcrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZZcrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZZcr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZZcrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZZcrControlDialog::GetSelectedMints()
{
...
void ZZcrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
