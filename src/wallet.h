...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ZCRat the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zZCRsend
enum ZerocoinSpendStatus {
    ZZCR_SPEND_OKAY = 0,                            // No error
    ZZCR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZZCR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZZCR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZZCR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZZCR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZZCR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZZCR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZZCR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZZCR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZZCR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZZCR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZZCR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZZCR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZZCR_SPENT_USED_ZZCR= 14,                      // Coin has already been spend
    ZZCR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZZCR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZZCROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzZCRWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzZCRTracker> zzcrTracker;
    ...
    void setZWallet(CzZCRWallet* zwallet)
    {
        zwalletMain = zwallet;
        zzcrTracker = std::unique_ptr<CzZCRTracker>(new CzZCRTracker(strWalletFile));
    }

    CzZCRWallet* getZWallet() { return zwalletMain; }
    ...
    /** zZCRreset */
    boost::signals2::signal<void()> NotifyzZCRReset;
