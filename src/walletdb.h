...
public:
    ...
    bool WriteZZCRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZZCRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZZCRSeed_deprecated(uint256& seed);
    bool EraseZZCRSeed();
    bool EraseZZCRSeed_deprecated();

    bool WriteZZCRCount(const uint32_t& nCount);
    bool ReadZZCRCount(uint32_t& nCount);
